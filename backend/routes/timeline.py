...
async def get_timeline_events(uid: str, mode: str = "academic"):
    ...
    try:
        ...
        if mode == 'side-hustle':
             # 1. Roadmaps Generated
            roadmaps_ref = user_ref.collection("roadmaps").order_by("last_updated", direction=firestore.Query.DESCENDING).limit(2)
            for doc in roadmaps_ref.stream():
                roadmap = doc.to_dict()
                created_at = roadmap.get('last_updated', datetime.now())
                ...
                events.append({
                    ...
                    "type": "roadmap",
                    "icon": "Book",
                    "title": f"Roadmap Created: {roadmap.get('skill', 'New Skill').title()}",
                    "description": "Customized learning path generated by AI.",
                    "time": get_time_ago(created_at),
                    "date": "Today" if is_today(created_at) else "Yesterday",
                    "details": [
                        f"{len(roadmap.get('phases', []))} Phases",
            ...
            # 2. Projects Generated (Assigned)
            projects_ref = user_ref.collection("projects").order_by("created_at", direction=firestore.Query.DESCENDING).limit(2)
            for doc in projects_ref.stream():
                ...
                events.append({
                    ...
                    "description": project.get('title', 'Side Hustle Project'),
                    "time": get_time_ago(datetime.fromisoformat(project.get('created_at')) if project.get('created_at') else datetime.now()),
                    "date": "Today", # Simplified for now
                    "details": [
                        f"Difficulty: {project.get('difficulty')}",
                        f"Est. Time: {project.get('estimated_time')}",
                        f"XP Reward: {project.get('xp_reward')} XP"
                    ]
                })
                event_id += 1

        else:
            # Academic Mode Events (Original Logic)
