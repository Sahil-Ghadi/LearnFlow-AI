...
import { X, CheckCircle, AlertCircle, ArrowRight, Brain, Trophy } from 'lucide-react';
...
export function AssessmentModal({ isOpen, onClose, exam, onUpdate }: AssessmentModalProps) {
    ...
    const handleStart = async () => {
        ...
        try {
            const syllabusNames = exam.syllabus ? exam.syllabus.map((s: any) => s.name) : [];
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <motion.div
                ...
                className="w-full max-w-2xl bg-[#0a0a0a] border border-white/10 rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]"
            >
                {/* Header */}
                <div className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5">
                    <div className="flex items-center gap-2">
                        <Brain className="h-5 w-5 text-accent" />
                        <h2 className="font-bold text-white">Assessment</h2>
                    </div>

                    {step === 'quiz' && (
                        <div className={cn(
                            "px-3 py-1 rounded-full font-mono text-sm font-bold border",
                            timeLeft < 30 ? "bg-red-500/20 text-red-500 border-red-500/50 animate-pulse" : "bg-zinc-800 text-zinc-300 border-zinc-700"
                        )}>
                            {formatTime(timeLeft)}
                        </div>
                    )}
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <p className="text-zinc-400 max-w-md mx-auto">
                                        This set contains 10 questions based on your syllabus: <br />
                                        <span className="text-zinc-300 italic">{exam.subject}</span>
                                    </p>
                                </div>
                                <div className="bg-white/5 p-4 rounded-lg max-w-md mx-auto text-sm text-zinc-400">
                                ...
                                >
                                    {isGenerating ? "Generating Questions..." : "Start Assessment"}
                                </Button>
                            </motion.div>
                        )}
                            ...
                            >
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-start">
                                        <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Question {currentQIdx + 1} of {questions.length}</span>
                                        <span className="text-xs px-2 py-1 bg-white/5 rounded text-zinc-400">{questions[currentQIdx].topic_tag}</span>
                                    </div>
                                ...
                                <div className="space-y-3">
                                    {questions[currentQIdx].options.map((option, idx) => (
                                        <div
                                            ...
                                            className={cn(
                                                "p-4 rounded-xl border border-white/5 cursor-pointer transition-all hover:bg-white/5 flex items-center gap-3",
                                        ...
                                        >
                                            ...
                                            <span className={answers[questions[currentQIdx].id] === idx ? "text-white" : "text-zinc-300"}>
                                                {option}
                                            </span>
                                        </div>
                                    ))}
                                ...
                                <div className="flex justify-end pt-4">
                                    <Button
                                        ...
                                        className="gap-2"
                                    >
                                        {currentQIdx === questions.length - 1 ? 'Finish & Submit' : 'Next Question'}
                        ...
                        {step === 'result' && result && (
                            ...
                            >
                                <div className="relative inline-block">
                                    <svg className="h-32 w-32 rotate-[-90deg]">
                                        ...
                                        <circle
                                            ...
                                            className={result.accuracy >= 70 ? "text-green-500" : result.accuracy >= 40 ? "text-yellow-500" : "text-red-500"}
                                            strokeDasharray={351}
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                        <span className="text-3xl font-bold text-white">{Math.round(result.accuracy)}%</span>
                                ...
                                <div className="grid grid-cols-2 gap-4 max-w-sm mx-auto">
                                    ...
                                    <div className="bg-white/5 p-4 rounded-xl">
                                        <p className="text-2xl font-bold text-white">{Math.round(result.readiness)}%</p>
                                ...
                                <div className="flex gap-3 justify-center pt-4">
                                    ...
                                    <Button onClick={handleFinish} className="bg-white text-black hover:bg-zinc-200">
                                        Close
                                    </Button>
                                </div>
                            </motion.div>
